...
- **Double-jump (runners) + tagger +4%:** Runners get one mid-air double-jump per airborne period
  (`doubleJumpSpeed = 5f`, weaker than the 6.5 ground jump); the 2nd jump plays the FrontFlip anim.
  Taggers move a flat `taggerBaseSpeedMultiplier = 1.04x` at all times, with the late-game curve
  multiplying on top. Player-triggered only — runner bots flee via the graph and never press jump
  mid-air, so bot behavior is unchanged.
- **Rigged characters:** `TagArenaBootstrap.AttachCharacterModel` swaps the placeholder capsule for
  the raccoon (player) / pest_control (bots) rigged models + `CharacterAnimatorBridge`; `TagAgent`
  gates its procedural arm/body-pitch/landing presentation off when a rigged model loads
  (`proceduralBody`), keeping the capsule + full tint as a graphics-missing fallback (model gets
  emission-only glow).

**MotorState/controller reconciliation:** main-game removed wall-run, so `MotorState` has no
`WallRunning` (Mantling..WallHook shifted down one vs the `double-jump-roles` enum). The committed
`CharacterAnimator.controller` was generated by `BuildCharacterAnimator.cs`, which still mapped
`WallRunning=3, Mantling=4..WallHook=9` — mis-driving every state from Mantling on. Fixed the
builder to the live enum (removed the WallRunning state, `Mantling=3..WallHook=8`) and regenerated
the controller headless. Grounded/Sliding/Airborne (0/1/2) + the Flipping-gated FrontFlip were
already correct; the fix realigns Mantle/Vault/Climb/Ladder/Swing/WallHook.

**Ordering safety:** the airborne coyote check was reordered to `window && ConsumeBufferedJump()` so
an expired-coyote buffered press falls through to the new double-jump branch instead of being eaten.
Ground/coyote `PerformJump` clears `_jumpBufferDeadline` (no instant double-jump off the initial
press); `_doubleJumpUsed` (ground-reset only) blocks a 3rd jump; taggers (`CanDoubleJump=false`) get none.

**Feel-test pending (manual):** double-jump height/timing feel (5.0f up-velocity), the FrontFlip anim
reading cleanly on the 2nd jump, the rigged raccoon/pest_control models rendering + self-animating in
a live scene, and whether the 1.04x tagger pressure feels right in a human-played round.
