...
public sealed class TagRulesTests
{
    ...
    public IEnumerator Tag_OnContact_ConvertsRunnerToTaggerWithGrace()
    {
        ...
        Assert.AreEqual(Role.Tagger, runnerAgent.Role, "Runner should be converted to Tagger on contact with a Tagger.");
    ...
    public IEnumerator TaggedAgent_CannotTagAnyoneDuringGracePeriod()
    {
        ...
        // Move C into contact with B while B is still in its conversion grace.
        cGo.GetComponent<Rigidbody>().position = bGo.transform.position + new Vector3(0.3f, 0f, 0f);
        yield return new WaitForFixedUpdate();
        yield return new WaitForFixedUpdate();

        Assert.AreEqual(Role.Runner, c.Role, "A tagger still in conversion grace should not be able to tag anyone.");
    }
    ...
    public IEnumerator TagArenaScene_SpawnsWithCorrectRoleDistribution()
    {
        ...
        // TagRulesConfig.taggerCount defaults to 1 (player-only Tagger) while feel-testing.
        Assert.AreEqual(1, taggers, "Should start with exactly 1 tagger.");
        Assert.AreEqual(11, runners, "Should start with exactly 11 runners.");
