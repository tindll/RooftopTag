...
public sealed class CharacterMotor : MonoBehaviour
{
    ...
    // Time.time before which the swing branch of TryStartLadderOrSwingAttach is skipped (regrab cooldown).
    private float _swingRegrabUntil;
    ...
    private void TickSwing(float dt)
    {
        ...
        if (jumpRelease || _input.InteractPressed || botAutoRelease || hangCapReached)
        {
            ...
            _swingRegrabUntil = Time.time + config.swing.regrabCooldownSeconds;
    ...
    private bool TryStartLadderOrSwingAttach()
    {
        ...
        foreach (Collider col in hits)
        {
            ...
            if (_currentSwing is null && Time.time >= _swingRegrabUntil
